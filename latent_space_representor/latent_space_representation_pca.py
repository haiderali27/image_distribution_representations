...
for mvtec_class in mvtec_classes:
    ...
    if len(good_features)>0:
        latent_space2 = pca2.fit_transform(good_features)
        plot2D(f'{mvtec_class.capitalize()} Good', latent_space2, plot_type='PCA')
        latent_space3 = pca3.fit_transform(good_features)
        plot3D(f'{mvtec_class.capitalize()} Good', latent_space3, plot_type='PCA')

    if len(anomaly_features)>0:
        latent_space2 = pca2.fit_transform(anomaly_features)
        plot2D(f'{mvtec_class.capitalize()} Anomaly', latent_space2, plot_type='PCA')
        latent_space3 = pca3.fit_transform(anomaly_features)
        plot3D(f'{mvtec_class.capitalize()} Anomaly', latent_space3, plot_type='PCA')
    if len(good_features)>0 and len(anomaly_features)>0:
        combined = np.array(combined)
        latent_space2 = pca2.fit_transform(combined)
        plot2D(f'{mvtec_class.capitalize()} Combined Space', latent_space2, plot_type='PCA')
        latent_space3 = pca3.fit_transform(combined)
        plot3D(f'{mvtec_class.capitalize()} Combined Space', latent_space3, plot_type='PCA')
